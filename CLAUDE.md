...
- Draft stripping (distinct from ghosting): Slack Desktop's Drafts compose editor silently discards every non-`rich_text` top-level block when the user opens the draft to edit. A `[rich_text, section]` payload ships fine and the draft isn't tombstoned, but the user only ever sees the rich_text content. `validateBlocksShape` now rejects any non-`rich_text` top-level block with `invalid_blocks` for this reason - section/divider/header/context alongside rich_text is silent content loss, not a valid mix. Multi-paragraph prose with visual bullets should use a single `rich_text_section` with inline `\n` and literal `•`; multiple top-level `rich_text` blocks get flattened by Desktop before section-before-list absorption runs, so splitting won't give you paragraph breaks.
- Section-before-list absorption: `rich_text_list` immediately following a `rich_text_section` absorbs the section into its first bullet. Desktop flattens adjacent top-level `rich_text` blocks before this runs, so the absorption reaches across block boundaries too. `validateBlocksShape` rejects both shapes with `invalid_blocks`, pointing callers at the workarounds (insert a `rich_text_quote` or `rich_text_preformatted` between section and list, or collapse into a single `rich_text_section` with inline `\n` and literal `•`).
- Other draft gotchas: `blocks` is Block Kit JSON (rich_text only - the CLI rejects mixed arrays, see skill.go); `client_last_updated_ts` must be exactly 7 decimal places on update/delete (helper `padDraftTS`, both pads and truncates); `drafts.list` has no cursor pagination, only `limit` + `has_more`. Update/delete auto-fetch the draft via `drafts.list` so callers don't juggle the ts themselves.
