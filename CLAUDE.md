...
- `draft create` needs the two-client split too, but **conditionally**: only a name-shaped channel recipient builds the workspace client (`resolve.ChannelInputNeedsLookup`, which mirrors `ResolveChannel`'s early returns so client selection can't drift from resolution behavior); `drafts.create` always rides the session client. Until 2026-08-31 the one session client went to `NewResolver`, and on Enterprise Grid the org token answers `enterprise_is_restricted` on both `users.conversations` and `conversations.list` - so **no `#name`/bare-name channel could ever resolve**, always erroring `channel_not_found` with a browse-the-channel-list hint that pointed away from the real cause (that flattening is `channelResolveError`; `todo/resolver-errors-are-reported-as-channel-not-found.md`, priority raised). The failure was verified live 2026-08-31, and the fix the same day (a `#name` resolved via users.conversations on the workspace client, the draft landed on the same channel ID the report's ID-form control used, then deleted); the routing is pinned by distinct-credential mocks plus mutation testing. The split is conditional for the same reason `saved list` builds its public client only under `--enrich`: ID/URL recipients resolve locally and must keep working on an org session credential alone, and requiring `NewClient` unconditionally would fail `not_authed` in a multi-credential setup with no `SLACK_WORKSPACE` selected. **User (`@name`/email) resolution deliberately stays on the session client** to preserve the org-context behavior that already worked: moving it to a workspace credential is an unverified scope change that could narrow directory visibility or change a duplicated name's collision winner, and a workspace OAuth bot lacks `users:read.email`. Tests: `TestDraftCreate_TokenRouting` (forces the conversations.list fallback with an empty member page so both resolution endpoints' tokens are observed; fails on the one-client build with the live error), `_UserRecipient` (pins users on the session token), `TestDraftCreate_OrgOnlyCredential` (IDs and invalid_input precedence survive with no workspace credential; a name gets `not_authed`, not the old lie), `TestDraftCreate_SessionAuthHintSurvivesSplit` (drops of the authMethod capture/restore fail it; green pre-split by design). Two claims from the 2026-08-31 bug report did not hold: the failure was never silent (exit 1 plus a JSON error on stderr; the reporting harness discarded both), and the "top-level `channel_id: null`" output claim was jq manufacturing null for a key `formatDraft` never emits.
...
- **`enrichItems`' join on the fatal path is not observable from a test, and trying to assert it produces a flaky test that discriminates nothing.** Measured 2026-08-10 while writing `TestSavedListEnrich_FirstCauseSurvivesSiblingCancel`: a "handler A had already exited when Run returned" check fails on the **correct** implementation 126/200 runs (77/200 if `exited` is sampled first; 0/100 under `-race`, where the slower build merely hides it), and with `wg.Wait()` removed it fails 178/200 - it fails both ways. The cause is structural. Once the page fails, `cancel()` tears siblings down client-side in microseconds: the transport closes the connection synchronously and returns before the server's netpoll can wake the blocked handler, so the join window is narrower than the time the command needs to write its `_meta` trailer. Widening it does not help - 9 blocked handlers plus 30 workers queued on the semaphore still gave 1/200 on correct code while the mutated build passed 71% of the time, i.e. worse discrimination, because anything that lengthens the tail lengthens it equally for both builds (`cancel()` precedes the early return either way). So removing `wg.Wait()` is a mutation **nothing in the suite catches**, and that is accepted rather than papered over. Don't re-derive this; don't add the check back. The success-path join is covered incidentally by every enrich test (an early return would leave outcomes unpopulated and rows textless). What the cancel test does pin, all deterministic: first-cause preservation (mutation caught 100/100 - not racy at all, since the sibling only fails *because* of the cancel, so its write always lands second), that `cancel()` is actually called (5/5, three distinct messages), and that the error stays unclassified so the trailer remains resumable (5/5).
- `runWithMockSession` cannot be used for any test that needs a handler still running when the command returns: its `defer srv.Close()` blocks until handlers finish, so a held handler is always complete by the time the helper hands back. `cancelRunSavedList` (cmd/saved_cancel_test.go) is the pattern - the test body owns the server. Related: `r.ParseForm()` in a handler that means to block on `r.Context().Done()` is load-bearing, because the server only starts the background read that notices a hung-up client once the request body hits EOF.
...
- `auth login`'s stderr hint reads `SLACK_WORKSPACE`/`SLACK_WORKSPACE_ORG` via `os.Getenv`, not `cli.Workspace`: kong merges `--workspace` into the same field, and a one-shot flag on login is not the persistent default the hint describes. It matches by exact credentials map key - the same lookup `ResolveCredentials` does - against every stored workspace (a re-login can add workspaces without changing the default), so it never affirms a value a later command would reject. For `SLACK_WORKSPACE` it also never calls stale a value `ResolveCredentials` would accept (an `E` id there is legal, and the org-only fallback depends on that); `SLACK_WORKSPACE_ORG` counts as set only for a stored `E` id, since a `T` id there is a real credential of the wrong kind (`team_is_restricted` on internal APIs). Rows sort by team name then id. Until 2026-09-04 `DesktopLogin` ranged a map and the hint picked the first non-org entry, so the order and the one suggested workspace changed every run and read as "you have no workspace set" when the env was already right; it now lists every candidate because there was no basis for picking one. `--workspace` is team-ID only - `ResolveCredentials`, `auth status`, and `auth logout` all look the value up as the credentials map key, and nothing compares `TeamName`; the flag help used to promise "name or ID" (corrected 2026-09-04 rather than adding name matching, since a Grid org and its workspace can share a name).
...
- `search messages`/`search files` rewrite `from:@x`, `to:@x`, and `in:@x` to `<@Uxxx>` via `rewriteUserModifiers` (cmd/searchquery.go) before the query ships. Verified live 2026-09-21: Slack's search matches `from:@handle` and `from:<@Uxxx>` (identical rows) but returns an empty page, exit 0, no error, for `from:@Real Name`, `from:@Uxxx`, and `from:"Real Name"`, and `in:@` behaves the same; the CLI's help advertised `from:@user` with no caveat, so an agent read six empty pages as "these people never said it". The rewrite runs every `@` value, plus any quoted `from:`/`to:` value (a quoted `in:` value without `@` may be a channel and passes through), through `ResolveUser` (handle, display name, real name, email, ID all resolve; `<@U…>` and `#chan` values don't start with `@` and pass through), so `from:@Uxxx` gets fixed for free. Unquoted names are greedy to the next `key:` token or quoted token, then the longest resolving prefix wins - one `users.list` load, the rest are map lookups - so `from:@Alice Adams skypilot` needs no quotes. Unresolved names are fatal `user_not_found` with a hint naming the quoted and `<@Uxxx>` forms; a silent empty page was the bug. Not fixed: `ResolveUser`'s name path swallows the `users.list` error, so a rate limit or auth failure during resolution also reads as `user_not_found` (todo `resolver-errors-are-reported-as-channel-not-found.md`), and duplicate names take the first cache writer. `to:` returned no rows in any form against the peers tried, so its rewrite is by analogy with `from:`/`in:`, not verified.
