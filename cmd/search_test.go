...
func TestSearchMessages_Pagination(t *testing.T) {
	...
	// Cursor is a raw page number ("2"), not base64-encoded.
	if cursor != "2" {
		t.Errorf("expected cursor='2' (raw page number), got %q", cursor)
	}
...
func TestSearchMessages_XoxcTokenAutoPromotes(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/api/search.messages", func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(searchResponse(
			[]map[string]any{
				searchMatch("1.1", "found it", "U01", "tammer", "C01", "general", "https://x/1"),
			},
			1, 1, 1,
		))
	})

	// runWithMockSession sets SLACK_TOKEN=xoxc-test with no SLACK_USER_TOKEN.
	// The xoxc- auto-promotion should make search work without an explicit user token.
	out, err := runWithMockSession(t, mux, "search", "messages", "test query")
	if err != nil {
		t.Fatalf("search should succeed with xoxc- token via auto-promotion, got: %v", err)
	}
