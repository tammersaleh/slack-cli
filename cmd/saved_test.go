...
func runWithMockSession(t *testing.T, handler http.Handler, args ...string) (string, error) {
	t.Helper()
	return runWithMockSessionStdin(t, "", handler, args...)
}

// runWithMockSessionStdin also pipes the given string into cli stdin.
// Empty stdin leaves cli.in nil, so stdin-reading code sees a TTY in tests
// unless the test explicitly provides input.
func runWithMockSessionStdin(t *testing.T, stdin string, handler http.Handler, args ...string) (string, error) {
	t.Helper()
...
func TestSavedList_AutoEnrichChannelName(t *testing.T) {
	...
	item := parseJSON(t, lines[0])
	if item["channel_name"] != "general" {
		t.Errorf("expected channel_name='general' without --enrich, got %q", item["channel_name"])
	}
}
