...
//
// xoxc- session tokens are auto-promoted to UserToken when no explicit user
// token is set, since they support user-scoped operations like search.
func ResolveCredentials(path string, workspace string) (*ResolvedCredentials, error) {
	...
	if envBot != "" {
		return promoteXoxc(&ResolvedCredentials{
			BotToken:  envBot,
			UserToken: envUser,
			Cookie:    envCookie,
		}), nil
...
// promoteXoxc copies BotToken to UserToken when the bot token is an xoxc-
// session token and no explicit user token was provided. xoxc- tokens are
// user/session tokens that support user-scoped operations (search, etc.).
func promoteXoxc(rc *ResolvedCredentials) *ResolvedCredentials {
	if rc.UserToken == "" && strings.HasPrefix(rc.BotToken, "xoxc-") {
		rc.UserToken = rc.BotToken
	}
	return rc
}
